...
Good Change Tours:
	...
	• TEXT-OWNS-FOLLOWING-HUNKS: a text node narrates the contiguous run of hunks that come AFTER it (intro), not before. This is how the viewer highlights hunks when a reader clicks a paragraph. Always anchor narration with `{ before: <hunkId> }` for the first hunk it describes, never `{ after: <hunkId> }` of the last hunk - a text node placed after a hunk would not claim that hunk and would either be orphaned or silently claim the next one. Trailers are a rare exception (a closing wrap-up paragraph at the end of a section after every hunk has been narrated) and they do NOT claim the hunks above them.
	• Within a section, either of these shapes is fine; pick whichever fits the content:
		...
		- Interleaved: section heading -> intro paragraph -> a few related hunks -> another paragraph that pivots to the next sub-theme -> a few more hunks -> ... -> end of section. Reach for this when a section genuinely spans multiple sub-themes that read better with a short pivot sentence between them than as one undifferentiated block of hunks. Each mid-section paragraph should mark a real shift in what's being shown (a new concern, a new file cluster, a new design step) - don't add a paragraph just to break up the page or to gloss every hunk. A "pivot" stays inside the current section - it is NOT a cue to open a new section heading.
	• CRITICAL ordering rule: finish one section completely (heading + intro paragraph + ALL of its hunks, plus any in-section pivot paragraphs) before adding the NEXT section's heading. Never add a section heading until every hunk that belongs to the prior section is already in place. The wrong shape - heading, paragraph, NEXT heading, hunks - silently dumps the first section's hunks into the second section.
	...
	• Some PR hunks belong out of the tour entirely - **generated noise** like `dist/**` or `src/generated/**` that regenerates on every commit. Mark those via `changeTour_addExclusion`, which writes a `<!-- changetour:exclude ... -->` marker into the tour's appendix. Three input shapes: exact-range (`file` + `startLine` + `endLine`), whole-file (`file` only, literal path), and glob (`file` with `*`/`**`, no line bounds). For autogenerated directories prefer the glob form - per-hunk markers break on the next regeneration when ranges shift. Use exclusion ONLY for hunks that should be out of the tour entirely; default to Miscellaneous for anything that is simply uninteresting but representable - this includes **deleted files**, which the data model fully supports (use `startLine: 0, endLine: 0` as `getAvailablePRHunks` reports them). Deletions usually go in Miscellaneous; reach for exclusion only when the deletion is mechanical noise (e.g. a generated artifact being removed). When a candidate is borderline (could go either way), **list it and ask the user** before calling `changeTour_addExclusion` - exclusion is a curation decision, not a default. Use `changeTour_removeExclusion` to undo a marker.
...
NODE IDS: node IDs are content-derived hashes (e.g. `h-3a2f1b0c9d` for a hunk, `t-abc1234567` for a text node, `g-…` for a group). They stay stable across structural edits, so an ID from getCurrentTour remains valid through subsequent add / remove / reorder calls on OTHER nodes. The one exception is duplicate-content nodes: two text paragraphs with identical wording, or two hunks at the same file:lines, share a base ID with a `-{N}` suffix on the later occurrences. Removing the first occurrence in a duplicate set renumbers the rest. If you ever get "Node with id … was not found", call changeTour_getCurrentTour and retry against the refreshed IDs.
