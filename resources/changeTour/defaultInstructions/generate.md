...
4. Build the tour in this order: opening text node (1-3 sentences orienting the reader, drawing on the PR description) -> for each substantive section: add the section -> add a single short text node introducing the section (cover the WHY for the whole group, not each hunk individually) -> add the hunks underneath it in logical order. Add highlights on a hunk only when it is >20 lines AND a specific sub-range carries the point. For every hunk longer than ~20 lines (and any non-trivial hunk in **Miscellaneous**), also call changeTour_setHunkSummary with a one-sentence purpose label - readers see this inline in the hunk header instead of the generic auto-fallback (the first changed line of the patch). Finally, add a single **Miscellaneous** section at the end with one short text node noting the contents are mechanical, then drop the trivial hunks underneath.
