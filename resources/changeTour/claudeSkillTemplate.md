...
2. **Cover every hunk.** Read the PR diff with `gh pr diff <num>` (or, for an exact view, read the file directly). Group hunks into 2-5 logical sections that match the PR description. Mechanical hunks (whitespace, generated files, trivial reformats) go into a single trailing section titled **Miscellaneous** with one short text node explaining the section groups low-substance changes - do not narrate them individually.
	...
	A small number of PR hunks should not be in the tour at all - **deleted files** (their new-side range is `0-0` with no `+` content to embed) and **generated noise** (autogenerated typings, build output, anything under `dist/` or `src/generated/`). For those, write exclusion markers into the trailing **appendix** at the tail of the tour file. The appendix is introduced by a single invisible sentinel comment and contains nothing but markers:
	...
	<!-- changetour:exclude file="path/to/file.ts" lines="A-B" reason="..." -->     # exact hunk
	<!-- changetour:exclude file="path/to/file.ts" reason="..." -->                 # whole file
	<!-- changetour:exclude file="src/generated/**" reason="autogenerated" -->      # glob
	```

	If the sentinel `<!-- changetour:excluded-section -->` already exists, append your new marker(s) below the existing ones. If it doesn't exist yet, add the sentinel at the end of the file (separated by a blank line from the last tour node) and then your marker(s) below it.

	**Do NOT** wrap the appendix in a `## Excluded Changes` markdown heading. `##` headings are reserved for user-authored tour sections - a tour that legitimately needs a narration section titled "Excluded Changes" (e.g. for a PR that removes features) must be free to use that title without colliding with the appendix.

	`*` matches one path segment; `**` matches across segments. For autogenerated directories prefer the glob form - per-hunk markers break on the next regeneration when ranges shift. Default to **Miscellaneous** for anything that is uninteresting but representable. When a candidate is borderline, surface it to the user and ask whether to exclude (and which form would be cleanest) or put in Miscellaneous - exclusion is a curation decision the human owns. Note: a diff body containing a literal three-backtick line is no longer a reason to exclude; the serializer picks a longer fence length automatically.

3. **Group narration.** Prefer one short text node per logical change, even if that change spans multiple hunks. Avoid writing a separate text node for every individual hunk - long stretches of narration-hunk-narration-hunk are bad UX. Within a section, either shape is fine: (a) one intro text node followed by all the hunks underneath it (use this when the section is one coherent change); or (b) interleaved sub-groups - intro paragraph, a few related hunks, a second short paragraph that pivots to the next sub-theme, a few more hunks, and so on (use this when the section genuinely spans multiple sub-themes that read better with a pivot sentence between them than as one undifferentiated block). Each mid-section paragraph must mark a real shift; never add one just to gloss a single hunk. A "pivot" paragraph stays INSIDE the current section - it is not a new section heading. Always finish one section completely (heading + intro + ALL its hunks, plus any in-section pivots) before opening the next section heading.
...
3. For every entry in `missingInTour`:
   ...
   - Existing `<!-- changetour:exclude ... -->` markers already filter their hunks out of `missingInTour`. If a remaining entry looks like it should be out of the tour entirely (deleted file, file under an autogenerated directory), do NOT silently add a new marker - surface it to the user and ask whether to exclude it (and whether a whole-file or glob form would be cleaner) or place it in Miscellaneous.
